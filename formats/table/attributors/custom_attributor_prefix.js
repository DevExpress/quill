...
const CUSTOM_ATTRIBUTERS_TAG_INFO = {
  ...tableConfig.allowedTags.reduce(
    buildTagInfo(
      tableConfig.name,
      new Set([...TABLE_ATTRIBUTES, ...TABLE_STYLES]),
    ),
    {},
  ),
  ...cellConfig.allowedTags.reduce(
    buildTagInfo(
      cellConfig.name,
      new Set([...TABLE_CELL_ATTRIBUTES, ...TABLE_CELL_STYLES]),
    ),
    {},
  ),
};
...
export function getKeyNameWithCustomPrefix(tagName, keyName) {
  ...
  return tagInfo.keyNamesSet.has(keyName)
    ? `${tagInfo.name}_${keyName}`
    : keyName;
}
